...
def init():
    ...
    # Attach trace only once
    if _trace_id is None:
        _trace_id = up.variable_cache.trace_add("write", _output_parser)
...
def destroy():
    ...
    if _trace_id is not None:
        try:
            up.variable_cache.trace_remove("write", _trace_id)
        except Exception:
            pass
        _trace_id = None
...
def _output_parser(_, __, ___):
    ...
    # Remove dead widgets
    dead_keys = [k for k, lbl in value_labels.items() if not lbl.winfo_exists()]
    for k in dead_keys:
        del value_labels[k]
...
def show_cache_window(root: ctk.CTk, main_frame_hook: ctk.CTkScrollableFrame):
    global txt_frame
